...
<Typeset
  fontSizes={["3.56rem", "2.25rem", "1.75rem", "1.43rem", "1.25rem"]}
...
Headings of type B need the class `.b-header` to be applied to heading tags from `<h3>` to `<h6>`.

<Typeset
  fontSizes={["1.75rem", "1.43rem", "1.25rem", "1.125rem"]}
...
Captions can be rendered with the classes `.caption` and `small-caption` depending on the required size on the `<p>` tag.

<Typeset
  fontSizes={["0.80rem", "0.697rem"]}
  fontFamily="Source Sans Pro"
  fontWeight={400}
  style={{ color: "#333" }}
  sampleText="Caption"
